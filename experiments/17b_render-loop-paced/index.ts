// render-loop-paced — 17 with paced atlas decoders (setInterval at
// 30fps each) instead of flat-out continuous loops. 17 showed flat-out
// decoders starved rAF to 20fps; this checks whether the production
// pacing pattern recovers the 60fps budget.
...
interface PacedDecoder {
  ...
  /** Drive from the render loop's rAF tick. Feeds chunks up to the
   *  target index for the given elapsed time. Single clock source =
   *  the renderer; no setInterval competing for the event loop. */
  feedTo(elapsedMs: number, targetFps: number): void
  stop(): void
}

/** Decoder driven by the render loop (rAF). The renderer calls
 *  `feedTo(elapsedMs, targetFps)` per tick; the decoder feeds as
 *  many chunks as needed to keep up with that target, no more, no
 *  less. Single clock source. */
function makePacedDecoder(atlas: ProbeInput): PacedDecoder {
  ...
  return {
    ...
    feedTo(elapsedMs, targetFps) {
      if (cursor >= atlas.chunks.length) {
        return
      }
      const targetCursor = Math.min(
        atlas.chunks.length,
        Math.floor((elapsedMs * targetFps) / 1000) + 1,
      )
      while (cursor < targetCursor && decoder.decodeQueueSize < 4) {
        decoder.decode(atlas.chunks[cursor])
        cursor++
      }
    },
    stop: () => {
...
void main() { outColor = texture(uTex, vUv); }`,
  ...
  await new Promise<void>(resolveLoop => {
    function tick() {
      ...
      // Drive each decoder from this tick — one clock source, no
      // setInterval / setTimeout competing with rAF.
      const elapsedMs = frameStart - startWall
      for (const decoder of decoders) {
        decoder.feedTo(elapsedMs, targetDecoderFps)
      }
      ...
      // production would. Frame time measured here is CPU-side draw
      // submission, not GPU completion.
...
async function run(): Promise<void> {
  ...
  status(`creating K=${atlasCount} paced decoders (fed from rAF tick)...`)
  const decoders = atlases.map(makePacedDecoder)
  // No warm-up needed — decoders are driven from the render loop's
  // rAF tick directly. First few ticks may paint nothing while the
  // first VideoFrame is in flight; that's the price of single-clock.

  const cells = buildCellSpecs()
  status(`built ${cells.length} cell specs`)

  status(`BASELINE pass — render loop alone (no capture, no rebuild)...`)
  const baseline = await runRenderLoop(decoders, cells, params.runSeconds)
  status(
    `  baseline: ${baseline.renderedFps.toFixed(1)}fps, mean=${baseline.meanFrameTimeMs.toFixed(2)}ms, ` +
      `p95=${baseline.p95FrameTimeMs.toFixed(2)}ms, max=${baseline.maxFrameTimeMs.toFixed(2)}ms, ` +
      `<16ms=${(baseline.framesUnder16ms * 100).toFixed(0)}%, <33ms=${(baseline.framesUnder33ms * 100).toFixed(0)}%`,
  )

  // Reset decoder cursors so the contended pass starts fresh.
  for (const decoder of decoders) {
    decoder.stop()
  }
  const decodersContended = atlases.map(makePacedDecoder)

  status(`CONTENDED pass — render + capture + worker rebuild...`)
  const buildStart = performance.now()
  const build = rebuildInWorker(source)
  const [capture, contended, buildResponse] = await Promise.all([
    captureForSeconds(params.runSeconds),
    runRenderLoop(decodersContended, cells, params.runSeconds),
    build.done,
  ])
  build.terminate()
  const buildWallClock = (performance.now() - buildStart) / 1000

  for (const decoder of decodersContended) {
    decoder.stop()
  }

  status(
    `  contended: ${contended.renderedFps.toFixed(1)}fps, mean=${contended.meanFrameTimeMs.toFixed(2)}ms, ` +
      `p95=${contended.p95FrameTimeMs.toFixed(2)}ms, max=${contended.maxFrameTimeMs.toFixed(2)}ms, ` +
      `<16ms=${(contended.framesUnder16ms * 100).toFixed(0)}%, <33ms=${(contended.framesUnder33ms * 100).toFixed(0)}%`,
  )
  ...
  status("done.")
  reportResult("render-loop-paced", params, {
    baseline,
    contended,
